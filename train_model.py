...
def import_images():
    ...
    for file in filelist:
        train_list.append(img_to_array(load_img('training_images/{}'.format(file), target_size=(1000,1000), color_mode='grayscale')))
...
def train_model(img_list, label_list):
    ...
    model = Sequential()
    model.add(Conv2D(100, kernel_size=3, activation='relu', input_shape=(1000,1000,1), padding='valid'))
    model.add(Flatten())
